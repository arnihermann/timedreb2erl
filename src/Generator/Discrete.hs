module Generator.Discrete (transModel) where
...
withT f = withTemplate $ "discrete/" ++ f
...
transMsgSrvG id typedparameters stms =  do
    st <- get
    let statements = nestStm "TT" $ map (flip evalState st . transStm) stms
    return $ render $
...
transMain (Main instancedecls) = do
    ...
    return $ intercalate sep [decl, conf, msg]
  where sep = ",\n    "
...
transInstanceDecl (InstanceDecl typedvardecl vardecls constants) =
    ...
    in return $ firstUpper inst ++ " = spawn(fun() -> " ++ firstLower cls ++ "(list_to_atom(\"" ++ firstLower inst ++"\")) end)"
